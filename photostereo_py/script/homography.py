...
class ImageAligner():
    ...
    def realign(self)-> np.array:
        ...
        H, mask = cv.findHomography(features1.matched_pts, features0.matched_pts, cv.RANSAC, 10.0)
        if mask is not None:
            num_inliers = int(mask.sum())
            inlier_ratio = num_inliers / len(mask)
            logger.info(f"[Homography] Number of inliers used in homography: {num_inliers} / {len(mask)} ({inlier_ratio:.2%})")
        h, w, c = self.ref.shape
        warped = cv.warpPerspective(self.test_orig, H, (w, h), borderMode=cv.BORDER_CONSTANT, borderValue=(0, 0, 0, 0))
